...
resolve_bind_address (struct sockaddr *sa)
{
  ...
  al = address_list_lookup_host (opt.bind_address, LH_BIND | LH_SILENT);
  if (!al)
    {
...
connect_to_host (const char *host, int port)
{
  ...
  struct address_list *al = address_list_lookup_host (host, 0);
  ...
  if (address_list_connected_p (al))
    {
      ...
      address_list_release (al);
      al = address_list_lookup_host (host, LH_REFRESH);
      goto retry;
    }
  address_list_release (al);
...
#else
    result = select_s (fd + 1, rd, wr, NULL, &tmout);
#endif
#ifdef WINDOWS
...
#else
  ret = select_s (sock + 1, &check_set, NULL, NULL, &to);
#endif
...
static int
sock_read_internal (int fd, char *buf, int bufsize)
{
  int res;
  do
#if !defined(USE_WATT32)
...
static int
sock_write_internal (int fd, char *buf, int bufsize)
{
  int res;
  do
#if !defined(USE_WATT32)
...
static int
sock_poll_internal (int fd, double timeout, int wait_for)
{
  return select_fd (fd, timeout, wait_for);
}

static int
sock_peek_internal (int fd, char *buf, int bufsize)
{
  int res;
  do
  ...
  if (timeout)
    {
      ...
      if (info && info->imp->poller)
        ...
        test = sock_poll_internal (fd, timeout, wf);
      if (test == 0)
        errno = ETIMEDOUT;
      if (test <= 0)
        return false;
...
fd_read (int fd, char *buf, int bufsize, double timeout)
{
  ...
  if (info && info->imp->reader)
    ...
    return sock_read_internal (fd, buf, bufsize);
}
...
fd_peek (int fd, char *buf, int bufsize, double timeout)
{
  ...
  if (info && info->imp->peeker)
    ...
    return sock_peek_internal (fd, buf, bufsize);
}
  ...
  while (bufsize > 0)
    {
      ...
      if (info && info->imp->writer)
        ...
        res = sock_write_internal (fd, buf, bufsize);
      if (res <= 0)
        break;
      buf += res;
      bufsize -= res;
